...
In some cases this is not desired, so you can override the new Layer's parent by including extra specifiers to commands like ``=Add Layer``, such as ``Add Layer <LayerName> to <ParentLayerName>``, ``Add Layer <LayerName> to Parent`` (causes the new layer to treat the current's parent as its own), ``Add Layer <LayerName> to Grandparent`` or ``Add Layer <LayerName> to Parent +1`` (both do the same thing and you can go even further up by using +2, +3, etc), and finally ``Add Independent Layer <LayerName>`` to prevent it being removed automatically at all (uses [Scheme] as its parent). These variants work with ``=Toggle Layer`` as well. You can also use most of them with ``=Remove Layer``, such as a layer removing both itself and its parent with ``=Remove parent layer``

Note that "held" Layers (layer active only so long as are holding a button such as [Layer.Alt] from the ``L2 = Layer Alt`` example above) are NOT automatically removed when their parent is removed. They are instead automatically removed when the button "holding" it active (L2 in this case) is released.

There is also the more advanced Layer command Replace, such as ``Replace this layer with <LayerName>`` or ``Replace Parent/GrandParent/etc Layer with <LayerName>`` or  just``Replace <LayerName> layer with <LayerName>`` which can be used to simultaneously Remove and Add in a single Command. The new layer will be set to the same parent as the layer it replaced in this case.
