...
EHUDType hudTypeNameToID(const std::string& theName)
{
	struct NameToEnumMapper
	{
		...
		NameToEnumMapper()
		{
			...
			map.setValue("RECT",				eHUDType_Rectangle);
			map.setValue("BLOCK",				eHUDType_Rectangle);
			map.setValue("SQUARE",				eHUDType_Rectangle);
			map.setValue("ROUNDRECT",			eHUDType_RoundRect);
			map.setValue("ROUNDEDRECT",			eHUDType_RoundRect);
			map.setValue("ROUNDRECTANGLE",		eHUDType_RoundRect);
			map.setValue("ROUNDEDRECTANGLE",	eHUDType_RoundRect);
			map.setValue("CIRCLE",				eHUDType_Circle);
			map.setValue("DOT",					eHUDType_Circle);
			map.setValue("ELLIPSE",				eHUDType_Circle);
			map.setValue("CROSS",				eHUDType_Crosshair);
			map.setValue("CROSSHAIR",			eHUDType_Crosshair);
			map.setValue("RETICLE",				eHUDType_Crosshair);
			const size_t actualMapSize = map.size();
			DBG_ASSERT(actualMapSize == kMapSize);
		}
	};
