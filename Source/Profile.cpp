...
static void expandPropertyVars(u16 theSectionID, u16 thePropID, bool init)
{
	...
	do {
		...
		{
			const u16 aVarID = theVarsMap.findIndex(aVarName);
...
static void loadProfile(int theProfilesCanLoadIdx)
{
	...
	for(u16 aSectionID = 0; aSectionID < sSectionsMap.size(); ++aSectionID)
	{
		PropertyMap& aSection = sSectionsMap.vals()[aSectionID];
		for(u16 aPropID = 0; aPropID < aSection.size(); ++aPropID)
	...
	bool saveToFile)
{
	...
	const u16 aSectionID = sSectionsMap.findOrAddIndex(theSection);
	PropertyMap& aSection = sSectionsMap.vals()[aSectionID];
	const u16 aPropID = aSection.findOrAddIndex(thePropertyName);
