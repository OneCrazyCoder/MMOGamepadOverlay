...
struct ZERO_INIT(CopiedBitmapIcon)
{
	HBITMAP image;
	POINT targetTL;
	SIZE size;
...
struct ZERO_INIT(CopyRectRequest)
{
	...
	bool operator==(const CopyRectRequest& rhs) const
	{
		return
			...
			overlayID == rhs.overlayID &&
			menuID == rhs.menuID &&
			rectIdx == rhs.rectIdx &&
			labelCacheIdx == rhs.labelCacheIdx &&
			multiIconIdx == rhs.multiIconIdx;
	}
};
...
static bool sCopiedIconResetNeeded = false;
...
static void initCopiedBitmapIcon(DrawData& dd, CopiedBitmapIcon& theIcon)
{
		...
		(theIcon.size.cx != anIconSizeX || theIcon.size.cy != anIconSizeY) )
	{
		...
		theIcon.image = null;
		// When an icon size has changed, need to reset cache data and redraw
		sCopiedIconResetNeeded = true;
	}
	...
	DrawData& dd, int theCopiedBitmapID, const CopyRectRequest& theCopyRequest)
{
	...
	else
	{
		...
		StretchBlt(dd.hdc, aDstL, aDstT, aDstW, aDstH,
			sBitmapDrawSrc, 0, 0, aSrcW, aSrcH, SRCCOPY);
		SelectObject(sBitmapDrawSrc, hOldBitmap);
	}
	...
	bool ignoreRangeSuffix = false)
{
	...
	if( aLabelIcon && aLabelIcon->copyFromTarget )
	{
		...
		int anID = 1;
		for(int end = intSize(sCopiedIcons.size()); anID < end; ++anID)
		{
			if( sCopiedIcons[anID].hotspotID == aLabelIcon->hotspotID )
			{
	...
	if( !ignoreRangeSuffix )
	{
			...
			aRangeEndIdx > aRangeStartIdx )
		{
			...
			LONG aSrcWidth = 0;
			LONG aSrcHeight = 0;
			theCacheEntry.multiIcon.dstWidth = 0;
			...
			for(int i = aRangeStartIdx; i <= aRangeEndIdx; ++i)
			{
				...
				case eMenuItemLabelType_Bitmap:
					...
					aSrcHeight = max(aSrcHeight,
						sBitmapIcons[anIconEntry.bitmapIconID].size.cy);
					break;
				case eMenuItemLabelType_CopyRect:
					theCacheEntry.subLabels.push_back(anIconEntry);
					aSrcWidth +=
						sCopiedIcons[anIconEntry.copyIconID].size.cx;
					aSrcHeight = max(aSrcHeight,
						sCopiedIcons[anIconEntry.copyIconID].size.cy);
					break;
				default:
			...
			theCacheEntry.multiIcon.srcWidth = dropTo<u16>(aSrcWidth);
			theCacheEntry.multiIcon.srcHeight = dropTo<u16>(aSrcHeight);
			if( !theCacheEntry.subLabels.empty() &&
				theCacheEntry.multiIcon.srcWidth &&
				theCacheEntry.multiIcon.srcHeight )
			{ return; }
		}
	}
	...
	int theCacheEntryID, int theMultiIconIdx = -1)
{
	...
	// Track draw count for each destination rect for this overlay
	OverlayPaintState& ps = sOverlayPaintStates[dd.overlayID];
	if( ps.rectDrawCount.size() != ps.rects.size() )
		ps.rectDrawCount.resize(ps.rects.size());
	DBG_ASSERT(size_t(theRectIdx) < ps.rects.size());
	if( theMultiIconIdx < 0 )
		++ps.rectDrawCount[theRectIdx];
	...
	{
		// Request the copied bitmap be refreshed/created/drawn ASAP
		CopyRectRequest aCopyRequest;
		aCopyRequest.dstL = theDrawRect.left;
		aCopyRequest.dstT = theDrawRect.top;
		aCopyRequest.dstW = theDrawRect.right - theDrawRect.left;
		aCopyRequest.dstH = theDrawRect.bottom - theDrawRect.top;
		aCopyRequest.useClipRect = theCurrBorderSize > 0;
		if( aCopyRequest.useClipRect )
		{
			aCopyRequest.clipRect =
				sOverlayPaintStates[dd.overlayID].rects[theRectIdx];
		...
		aCopyRequest.rectIdx = dropTo<u16>(theRectIdx);
		aCopyRequest.labelCacheIdx = dropTo<u16>(theCacheEntryID);
		aCopyRequest.multiIconIdx = dropTo<s8>(theMultiIconIdx);
		std::vector<CopyRectRequest>& aCopyQueue = 
			sOverlayPaintStates[dd.overlayID].copyRectQueue;
		std::vector<CopyRectRequest>::iterator it = std::find(
			aCopyQueue.begin(), aCopyQueue.end(), aCopyRequest);
		if( it != aCopyQueue.end() )
		{
			it->lowPriority = false;
			it->drawCount = ps.rectDrawCount[theRectIdx];
		}
		else
		{
			aCopyRequest.lowPriority = false;
			aCopyRequest.drawCount = ps.rectDrawCount[theRectIdx];
			aCopyQueue.push_back(aCopyRequest);
		}
	...
	else if( theCacheEntry.type == eMenuItemLabelType_MultiIcon )
	{
		...
		DBG_ASSERT(theCacheEntry.multiIcon.srcWidth > 0);
		DBG_ASSERT(theCacheEntry.multiIcon.srcHeight > 0);
		// Multi-icons always draw to the inner rect (at max border size) only
		RECT aRect;
		aRect.left = theDrawRect.left + theMaxBorderSize;
		aRect.top = theDrawRect.top + theMaxBorderSize;
		aRect.right = theDrawRect.right - theMaxBorderSize;
		aRect.bottom = theDrawRect.bottom - theMaxBorderSize;
		if( aRect.right <= aRect.left ||
			aRect.bottom <= aRect.top )
		{ return; }

		if( theCacheEntry.multiIcon.dstWidth == 0 )
		{
			// Calculate true dest width to get icons adjacent and centered
			// Treat as a single wide bitmap and calculate where it would draw
			const int aSrcW = theCacheEntry.multiIcon.srcWidth;
			const int aSrcH = theCacheEntry.multiIcon.srcHeight;
			int aDstW = aRect.right - aRect.left;
			int aDstH = aRect.bottom - aRect.top;
			if( theMenuApp.itemType != eMenuItemType_Rect &&
				theMenuApp.itemType != eMenuItemType_Label )
			{// Don't expand to fill inner rect for non-rect-shaped menu items
				aDstW = min(aDstW, aSrcW);
				aDstH = min(aDstH, aSrcH);
			}
			const double aSrcAspectRatio = double(aSrcW) / aSrcH;
			const double aDstAspectRatio = double(aDstW) / aDstH;
			if( aSrcAspectRatio < aDstAspectRatio )
				aDstW = min(aDstW, int(aDstH * aSrcAspectRatio));
			theCacheEntry.multiIcon.dstWidth = aDstW;
		}
		const LONG aTotalSrcWidth = theCacheEntry.multiIcon.srcWidth;
		const LONG aDestWidth = theCacheEntry.multiIcon.dstWidth;
		aRect.left = (aRect.left + aRect.right - aDestWidth) / 2;
			...
			i < end; ++i)
		{
			...
			aRect.right += aSrcWidth * aDestWidth / aTotalSrcWidth;
			drawMenuItemLabel(dd, aRect, theRectIdx, theLabel,
				theMenuApp, theItemApp, 0, 0, theCacheEntryID, i);
		}
	}
}
	...
	int theCacheEntryID)
{
	...
	DBG_ASSERT(size_t(theRectIdx) < ps.rects.size());
	drawMenuItemBG(dd, menuApp, itemApp, ps.rects[theRectIdx]);
...
static void drawBasicMenu(DrawData& dd)
{
	...
	for(int itemIdx = 0; itemIdx < anItemCount; ++itemIdx)
	{
				...
				(itemIdx == ps.prevFlashing || itemIdx == ps.flashing)) )
		{
			...
			{
				drawMenuItem(dd, itemIdx+1, itemIdx,
...
static void drawSlotsMenu(DrawData& dd)
{
		...
		(selectionChanged || shouldRedrawAll) )
	{
		...
		if( !anAltLabel.empty() )
		{
			...
			const MenuItemAppearance& itemApp =
				getMenuItemAppearance(dd.menuID, dd.itemDrawState);
			drawBGRect(dd, itemApp, ps.rects.back());
			drawMenuItemLabel(
				dd,
				ps.rects.back(),
				intSize(ps.rects.size())-1,
				anAltLabel,
				menuApp, itemApp,
				itemApp.borderSize,
				getMaxBorderSize(dd.menuID),
		...
		compIdx = (compIdx + 1) % anItemCount)
	{
			...
			(isSelection && flashingChanged) )
		{
			...
			drawMenuItem(dd, compIdx+1, itemIdx,
				InputMap::menuItemLabel(dd.menuID, itemIdx),
				itemIdx + hasTitle);
...
static void drawHUDElement(DrawData& dd)
{
	...
	drawMenuItem(dd, 0, 0, InputMap::menuItemLabel(dd.menuID, 0), 0);
}
...
static const int getCopyRectRequestIconID(
	int theOverlayID, int theMenuID,
	const CopyRectRequest& theRequest)
{
	if( theRequest.overlayID != theOverlayID )
		return 0;
	const OverlayPaintState& ps = sOverlayPaintStates[theOverlayID];
	if( theRequest.menuID != theMenuID )
		return 0;
	if( size_t(theRequest.rectIdx) >= ps.rectDrawCount.size() )
		return 0;
	if( theRequest.drawCount != ps.rectDrawCount[theRequest.rectIdx] )
		return 0;
...
static void resetCopiedIconSizes()
{
...
void cleanup()
{
	...
	for(int i = 0, end = intSize(sBitmapFiles.size()); i < end; ++i)
		DeleteObject(sBitmapFiles[i].handle);
	for(int i = 0, end = intSize(sCopiedIcons.size()); i < end; ++i)
		DeleteObject(sCopiedIcons[i].image);
...
void update()
{
	...
	if( sCopiedIconResetNeeded )
		resetCopiedIconSizes();
	...
	else if( gAppRunTime >= sAutoRefreshCopyRectTime )
	{
			...
			itr != sAutoRefreshCopyRectQueue.end(); itr = next_itr)
		{
			...
			if( !getCopyRectRequestIconID(itr->overlayID, aMenuID, *itr) )
...
void updateScaling()
{
	...
	// Re-init copy rect icon data since they are also affected by scale
